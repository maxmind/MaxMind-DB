...
// top. This is the per-occurrence total used by the flat payload-budget fixtures;
// it does not imply that a memoizing reader materializes each target again. The
// function reads the generated bytes rather than recomputing the builder's
// arithmetic, so a change to the encoding or element count fails the assertion.
